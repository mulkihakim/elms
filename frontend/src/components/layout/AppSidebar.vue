...
// Daftar menu berdasar role sesuai dokumen perencanaan (DESIGN.md § 5)
const menuSections = computed(() => {
    ...
    return [
      {
        ...
        items: [
          ...
          { name: 'Profil Saya', path: '/profile', icon: 'pi pi-user' },
        ],
      },
      {
  ...
  // EMPLOYEE (tanpa Dashboard sesuai DESIGN.md § 5)
  return [
    {
      title: 'Utama',
      items: [{ name: 'Profil Saya', path: '/profile', icon: 'pi pi-user' }],
    },
    {
...
<template>
  ...
  <!-- Sidebar Container -->
  <aside
    class="fixed lg:static top-16 lg:top-0 bottom-0 left-0 z-40 w-64 h-[calc(100vh-4rem)] lg:h-full bg-white border-r border-slate-200 flex flex-col transition-all duration-200 ease-in-out shrink-0"
  ...
  >
    ...
    <div class="p-3 border-t border-slate-100 text-[11px] text-slate-400 text-center shrink-0">
      ELMS &copy; 2026
    </div>
  </aside>
</template>
